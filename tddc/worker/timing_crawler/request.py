...
class Request(object):
    ...
    def __init__(self, task):
        self.timeout = getattr(task, 'i_timeout', 10)
        allow_redirects = getattr(task, 'b_allow_redirects', True)
        self.allow_redirects = allow_redirects
        self.method = task.s_method or 'get'
        self.url = task.s_url
        self.headers = self.make_headers(task)
        self.cookies = getattr(task, 's_cookies', None)
        self.proxies = ProxyHelper.to_borrow_proxy(task)
    ...
    def __call__(self, *args, **kwargs):
        kwargs = {
            'timeout': self.timeout, 'allow_redirects': self.allow_redirects, 'method': self.method,
            'url': self.url, 'headers': self.headers, 'cookies': self.cookies, 'params': self.params,
            'data': json.loads(self.data) if self.data else None,
            'json': json.loads(self.json) if self.json else None,
        }
        if self.proxies:
            kwargs['proxies'] = {
